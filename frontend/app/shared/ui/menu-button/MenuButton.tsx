...
function MenuButton({ handleClick, variant, children, text, bgColor = 'gray' }: MenuButtonProps) {
    ...
    const setColor = (color: string) => {
        ...
        if (color === 'red') return 'bg-error/10 text-error hover:bg-error/25 active:bg-error/5';
        if (color === 'gray') return 'bg-white/10 text-white hover:bg-white/25 active:bg-white/5';
    };
